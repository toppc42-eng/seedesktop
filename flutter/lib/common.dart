...
class MyTheme {
  ...
  /// ירוק־טורקיז — כפתורי פעולה ראשיים (מיושר לדף הנחיתה / מיתוג).
  static const Color accent = Color(0xFF36BA9B);
  static const Color accent50 = Color(0x7736BA9B);
  static const Color accent80 = Color(0xAA36BA9B);
  static const Color canvasColor = Color(0xFF212121);
  static const Color border = Color(0xFFCCCCCC);
  static const Color idColor = Color(0xFF2EAE94);
  static const Color darkGray = Color.fromARGB(255, 148, 148, 148);
  static const Color cmIdColor = Color(0xFF21790B);
  static const Color dark = Colors.black87;
  static const Color button = Color(0xFF36BA9B);
...
bool matchPeerSync(
    String searchText, Peer peer, PeerTabIndex peerTabIndex) {
  if (searchText.isEmpty) {
    return true;
  }
  if (peer.id.toLowerCase().contains(searchText)) {
...
/// Clears [IS_TERMINAL_ADMIN] so the next terminal session uses the normal
/// SeeDesktop password flow (like remote desktop), not the Windows admin
/// username/password dialog. Call this before any **non–run-as-admin** terminal
/// connection; the env can leak from a previous `--terminal-admin` / local admin flow.
void clearEnvTerminalAdmin() {
  bind.mainSetEnv(key: 'IS_TERMINAL_ADMIN', value: null);
}
...
Future<void> releaseAllLocalSessionsAndCloseWindows() async {
  final map = await loadPeerSessionMapPublic();
  for (final peerId in map.keys.toList()) {
    await releaseConnectionForPeer(peerId);
  }
  await releaseConnectionFromPrefs(force: true);
  if (isDesktop) {
    try {
    ...
    if (context.mounted) {
      await showDialog<void>(
        ...
        builder: (ctx) => AlertDialog(
          ...
          content: const Text(
            'File transfer is available only with a PRO license.\n'
            'Please upgrade to unlock this feature.',
          ),
          actions: [
            TextButton(
              onPressed: () => launchUrl(Uri.parse(kBuyNowUrl)),
  ...
  if (tier != LocalLicenseTier.proActive) {
    if (await hasActiveLicenseSessionToOtherPeer(id)) {
      ...
      if (otherPeerId != null && context.mounted) {
        final proceed = await showFreeTierSwitchConnectionConfirmDialog(
          context,
          otherPeerId,
          id,
        );
      ...
      await releaseAllLocalSessionsAndCloseWindows();
    }
    await markPendingLicenseConnectionPeer(id);
  }
...
String localLicenseTierShortLabel(LocalLicenseTier tier) {
  switch (tier) {
    case LocalLicenseTier.proActive:
      return 'PRO';
    case LocalLicenseTier.free:
      return 'FREE';
    case LocalLicenseTier.proExpired:
      return 'PRO-EXPIRED';
    case LocalLicenseTier.unlicensed:
      return 'UNLICENSED';
  }
}

Future<String> getWindowNameWithIdAndLicense(String id,
    {WindowType? overrideType}) async {
  final base = getWindowNameWithId(id, overrideType: overrideType);
  final tier = await getLocalLicenseTier();
  if (tier == LocalLicenseTier.proActive) {
    final savedLicense = (await getSavedLicenseKey())?.trim() ?? '';
    if (savedLicense.toUpperCase().startsWith('SD-PRORMM-')) {
      return '$base [${maskLicense(savedLicense)}]';
    }
  }
  return '$base [${localLicenseTierShortLabel(tier)}]';
}
