    ...
    PopupMenuButton<String>(
        ...
        itemBuilder: (context) {
          ...
          return [
            ...
            if (showPasswordOption && !isChangePermanentPasswordDisabled())
              PopupMenuItem(
            ...
            if (showPasswordOption)
              PopupMenuItem(
                ...
                child: listTile('Use permanent password', true),
              ),
          ];
        },
...
class _ServerInfoState extends State<ServerInfo> {
  ...
  Widget build(BuildContext context) {
    ...
    final maskedLicense = (_savedLicense == null || _savedLicense!.trim().isEmpty)
        ? null
        : maskLicense(_savedLicense!.trim());
    return PaddingCard(
        ...
        child: Column(
          ...
          children: [
            ...
            // One-time password display intentionally hidden.
            const SizedBox(height: 8),
            ConnectionStateNotification(),
            const SizedBox(height: 12),
            Row(children: [
              const Icon(Icons.vpn_key_outlined, color: Colors.grey, size: iconSize)
                  .marginOnly(right: iconMarginRight),
              Text(
                'License',
                style: textStyleHeading,
            ...
            Container(
              ...
              margin: const EdgeInsets.only(left: 39, top: 6),
              child: maskedLicense != null
                  ? FutureBuilder<bool>(
                      future: shouldAllowLicenseDisplayCopyLocal(),
                      builder: (context, snap) {
                        final allow = snap.data == true;
                        final w = Text(
                          maskedLicense,
                        ...
                        return allow
                            ? w
                            : SelectionContainer.disabled(child: w);
                      },
                    )
                  : Wrap(
...
class _PermissionCheckerState extends State<PermissionChecker> {
  ...
  Widget build(BuildContext context) {
    ...
    final isConnected = serverModel.connectStatus > 0;
    return PaddingCard(
            ...
            children: [
              ...
              Expanded(
                child: Text(
                  ...
                  style: const TextStyle(fontSize: 13, color: MyTheme.darkGray),
                ),
              ),
              ElevatedButton.icon(
                style: ButtonStyle(
                  backgroundColor: WidgetStateProperty.all(
                    serverModel.mediaOk ? Colors.red : MyTheme.accent,
                  ),
                ),
                icon: Icon(serverModel.mediaOk ? Icons.stop : Icons.play_arrow),
                onPressed: serverModel.toggleService,
                label: Text(
                    translate(serverModel.mediaOk ? "Stop service" : "Start service")),
              ),
            ],
          ).marginOnly(bottom: 8),
          PermissionRow(
