...
import '../../consts.dart';
import '../../desktop/widgets/see_desk_login_dialog.dart';
...
class _SettingsState extends State<SettingsPage> with WidgetsBindingObserver {
  ...
  String _licenseStatusLabel = 'Unlicensed';
  ...
  Widget build(BuildContext context) {
    ...
    final settings = SettingsList(
      sections: [
        ...
        SettingsSection(
          title: Text(translate('SeeDesktop License')),
          tiles: [
            SettingsTile(
              title: Text(translate('License Key')),
              description: Text(_licenseStatusLabel),
              leading: Icon(Icons.verified_user),
              trailing: Icon(Icons.arrow_forward_ios),
              onPressed: (context) => _openLicenseDialog(),
            ),
            SettingsTile(
              title: Text(translate('Clear License Key')),
              leading: Icon(Icons.delete_outline),
              onPressed: (context) => _clearLicenseWithConfirmation(),
            ),
            if (_isExpiredProLicense)
              SettingsTile(
                title: const Text('Renew Now'),
                description:
                    const Text('License expired. Renew to restore Pro access.'),
                leading: const Icon(Icons.warning_amber_rounded,
                    color: Colors.redAccent),
                onPressed: (context) => launchUrlString(kBuyNowUrl),
              ),
            if (_isExpiredProLicense)
              SettingsTile(
                title: const Text('Refresh Status'),
                description: const Text(
                    'Tap after renewal to unlock Pro immediately.'),
  ...
  Future<void> _refreshLicenseStatus() async {
    ...
    if (!mounted) return;
    setState(() {
      _licenseStatusLabel = nextStatus;
  ...
  Future<void> _clearLicenseWithConfirmation() async {
    final confirmed = await gFFI.dialogManager.show<bool>(
      (setState, close, context) {
        ...
        return CustomAlertDialog(
          title: Text(translate('Clear License Key')),
          content: Text(
              'This will remove the saved SeeDesktop license from this device.'),
          actions: [
            dialogButton('Cancel', onPressed: close, isOutline: true),
            dialogButton('Clear', onPressed: submit),
          ],
          onSubmit: submit,
          onCancel: close,
        );
