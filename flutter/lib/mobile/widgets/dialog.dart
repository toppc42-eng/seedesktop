...
String _inferHostFromApiServer(String apiServer) {
  final normalized = apiServer.trim();
...
void showServerSettings(OverlayDialogManager dialogManager,
  ...
  if (lockedServer.isNotEmpty) {
    showToast('Network settings are managed by administrator');
    return;
  }
  const defaultKey =
  ...
  try {
    ...
    if (kDebugMode) debugPrint("Invalid server config: $e");
  }
  ...
  final configuredApiServer = (await bind.mainGetApiServer()).trim();
  final inferredHost = _inferHostFromApiServer(configuredApiServer);
  final fallbackApiServer = _firstNonEmpty([
    configuredApiServer,
    'https://api.seedesktop.com',
  ]);
  final key = await bind.mainGetOption(key: 'key');

  options['custom-rendezvous-server'] = _firstNonEmpty([
    idServer,
    options['custom-rendezvous-server']?.toString(),
    inferredHost,
  ]);
  options['relay-server'] = _firstNonEmpty([
    relayServer,
    options['relay-server']?.toString(),
    inferredHost,
  ]);
  options['api-server'] = _firstNonEmpty([
    apiServer,
    options['api-server']?.toString(),
    fallbackApiServer,
  ]);
  options['key'] =
      _firstNonEmpty([key, options['key']?.toString(), defaultKey]);
