...
class FileController {
  ...
  /// Download a remote file to a temporary directory and mark it as a print job.
  Future<void> printRemoteFile(Entry from) async {
    if (isLocal) return; // Only from remote
    if (!from.isFile) return;

    final tempDir = await getTemporaryDirectory();
    final tempPath = tempDir.path;

    final jobID = jobController.addTransferJob(from, true, isPrintJob: true);
    
    // We construct the local target path using the local OS path separator
    final toPath = PathUtil.join(tempPath, from.name, Platform.isWindows);

    // Save the destination path in the job so we can open it later
    final jobIndex = jobController.getJob(jobID);
    if (jobIndex != -1) {
      jobController.jobTable[jobIndex].to = tempPath;
    }
    ...
    await Future.forEach(items.items, (Entry item) async {
      ...
      for (var i = 0; i < entries.length; i++) {
        ...
        try {
          ...
          if (kDebugMode) debugPrint("remove error: $e");
        }
      }
    });
    refresh();
...
class JobController {
  ...
  // return jobID
  int addTransferJob(Entry from, bool isRemoteToLocal, {bool isPrintJob = false}) {
    final jobID = JobController.jobID.next();
  ...
  Future<bool> jobDone(Map<String, dynamic> evt) async {
    ...
    if (job.isPrintJob && job.state == JobState.done) {
      final localPath = PathUtil.join(job.to, job.fileName, Platform.isWindows);
