...
class PeerTabModel with ChangeNotifier {
  ...
  static const int maxTabCount = 11;
  ...
  PeerTabModel(this.parent) {
    ...
    // Product requirement:
    // - Recent must be the first top navigation item.
  ...
  void _ensureLocalMaintenanceAfterLan() {
    ...
    list.remove(lm);
    final i = list.indexOf(lan);
    if (i >= 0) {
      list.insert(i + 1, lm);
    } else {
      list.insert(1, lm);
    }
    if (list.length != orders.length) return;
  ...
  void _ensureRecentIsFirstInOrder() {
    final recentTab = PeerTabIndex.recent.index;
    final list = orders.toList();
    list.remove(recentTab);
    list.insert(0, recentTab);
  ...
  void _ensureRecentVisible() {
    final recentTab = PeerTabIndex.recent.index;
    if (_isVisible[recentTab]) return;
    _isVisible[recentTab] = true;
    try {
