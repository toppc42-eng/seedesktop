...
class ServerModel with ChangeNotifier {
  ...
  ServerModel(this.parent) {
    ...
    Future.microtask(_enforcePermanentPasswordOnly);
    Future.microtask(_enforceOpenConnectionsInTabs);
  ...
  Future<void> _ensureDefaultFullAccessPermissions() async {
    ...
    await setIfEmpty(kOptionVerificationMethod, kUsePermanentPassword);
  }

  Future<void> _enforcePermanentPasswordOnly() async {
    final current = await bind.mainGetOption(key: kOptionVerificationMethod);
    if (current != kUsePermanentPassword) {
      await bind.mainSetOption(
          key: kOptionVerificationMethod, value: kUsePermanentPassword);
    }
  }

  Future<void> _enforceOpenConnectionsInTabs() async {
    if (!isDesktop || isWeb) return;
    await bind.mainSetLocalOption(key: kOptionOpenNewConnInTabs, value: 'Y');
  }
  ...
  Future<void> startService() async {
    ...
    // Force a permission/service state refresh after startup.
  ...
  void sendLoginResponse(Client client, bool res) async {
    if (res) {
      ...
      if (!client.isFileTransfer && !client.isTerminal) {
        unawaited(_ensureCaptureStarted());
      }
      parent.target?.invokeMethod("cancel_notification", client.id);
  ...
  Future<void> _ensureCaptureStarted() async {
    if (!isAndroid) return;
    // Connection can be authorized before media/service state is fully ready.
    // Retry briefly to avoid "Waiting for image" stalls on the remote side.
    for (var attempt = 0; attempt < 8; attempt++) {
      try {
        final started = await parent.target?.invokeMethod("start_capture");
        if (started == true) {
          return;
        }
        await parent.target?.invokeMethod("check_service");
      } catch (_) {
        // Ignore and keep retrying for transient timing races.
      }
      await Future.delayed(Duration(milliseconds: 350));
    }
    debugPrint("start_capture failed after retries");
  }
