...
class FfiModel with ChangeNotifier {
  ...
  bool _autoPreviewCapturePending = false;
  bool _manualThumbnailCapturePending = false;
  String _manualThumbnailOutputPath = '';
  Completer<bool>? _manualThumbnailCaptureCompleter;
  ...
  clear() {
    ...
    _autoPreviewCapturePending = false;
    _manualThumbnailCapturePending = false;
    _manualThumbnailOutputPath = '';
    if (!(_manualThumbnailCaptureCompleter?.isCompleted ?? true)) {
      _manualThumbnailCaptureCompleter?.complete(false);
    }
    _manualThumbnailCaptureCompleter = null;
  }

  Future<void> capturePeerDesktopPreviewIfNeeded(String peerId) async {
    if (_autoPreviewCapturePending || kIsWeb || peerId.isEmpty) return;
    _autoPreviewCapturePending = true;
    try {
      await _applyRemoteWallpaperPreview(peerId);
    } finally {
      _autoPreviewCapturePending = false;
    }
  }

  Future<String> _thumbnailPathForPeer(String peerId) async {
    final appDir = await getApplicationSupportDirectory();
    final thumbnailDir = Directory(p.join(appDir.path, 'previews'));
    if (!await thumbnailDir.exists()) {
      await thumbnailDir.create(recursive: true);
    }
    final safePeerId = peerId.replaceAll(RegExp(r'[^A-Za-z0-9._-]'), '_');
    return p.join(thumbnailDir.path, '$safePeerId.png');
  }

  Future<void> _reloadPeerLists() async {
    await bind.mainLoadRecentPeers();
    await bind.mainLoadFavPeers();
    await bind.mainLoadLanPeers();
  }

  Future<void> _applyRemoteWallpaperPreview(String peerId) async {
    final payload = _pi.platformAdditions['remote_wallpaper_jpeg_b64'];
    final b64 = payload is String ? payload.trim() : '';
    final rawStation = _pi.platformAdditions['remote_wallpaper_station_number'];
    final safePeerId = _sanitizeStationNumber(peerId);
    final safePayloadStation =
        _sanitizeStationNumber(rawStation is String ? rawStation : '');
    // Strict isolation: always bind preview file to the connection's station id.
    // If payload station is missing/mismatched, fallback to peerId to prevent overwrite.
    final stationNumber = safePayloadStation.isNotEmpty &&
            safePayloadStation != 'unknown_station' &&
            safePayloadStation == safePeerId
        ? safePayloadStation
        : safePeerId;
    if (b64.isEmpty) {
      await bind.mainSetPeerOption(
          id: peerId, key: 'desktop-preview-path', value: '');
      await _reloadPeerLists();
      return;
    }
    try {
      final bytes = base64Decode(b64);
      if (bytes.length < 256) {
        throw Exception('Wallpaper payload is too small');
      }
      final thumbnailDir = await _getStationImagesCacheDir();
      if (!await thumbnailDir.exists()) {
        await thumbnailDir.create(recursive: true);
      }
      final outputPath = p.join(thumbnailDir.path, '$stationNumber.jpg');
      await File(outputPath).writeAsBytes(bytes, flush: true);
      final cacheBuster = DateTime.now().millisecondsSinceEpoch;
      final fileUri = '${Uri.file(outputPath)}?t=$cacheBuster';
      await bind.mainSetPeerOption(
          id: peerId, key: 'desktop-preview-path', value: fileUri);
      await _reloadPeerLists();
    } catch (_) {
      await bind.mainSetPeerOption(
          id: peerId, key: 'desktop-preview-path', value: '');
  ...
  Future<Directory> _getStationImagesCacheDir() async {
    if (Platform.isWindows) {
      final appData = Platform.environment['APPDATA'] ?? '';
      if (appData.trim().isNotEmpty) {
        return Directory(
            p.join(appData, 'SeeDesktop', 'Cache', 'StationImages'));
      }
    }
    final appDir = await getApplicationSupportDirectory();
    return Directory(
        p.join(appDir.path, 'SeeDesktop', 'Cache', 'StationImages'));
  }

  String _sanitizeStationNumber(String input) {
    ...
    if (value.isEmpty) {
      value = 'unknown_station';
    }
    return value;
  }

  Future<bool> _waitForValidImageFile(String path,
      ...
      try {
        if (await file.exists()) {
          ...
          if (size > 128) {
            ...
            if (bytes.length > 128 && img2.decodeImage(bytes) != null) {
  ...
  Future<bool> _saveScreenshotToPeerPreview(
      SessionID sessionId, String peerId, String outputPath) async {
    final res = await bind.sessionHandleScreenshot(
        sessionId: sessionId, action: '0:$outputPath');
    if (res.isNotEmpty) {
      return false;
    }
    final valid = await _waitForValidImageFile(outputPath);
    if (!valid) {
      try {
        final brokenFile = File(outputPath);
    ...
    await bind.mainSetPeerOption(
        id: peerId, key: 'desktop-preview-path', value: outputPath);
    await _reloadPeerLists();
    return true;
  }

  Future<bool> captureAndSavePeerThumbnail(String peerId) async {
    if (kIsWeb || peerId.isEmpty) return false;
    if (parent.target?.connType != ConnType.defaultConn) return false;
    final isScreenshotSupported = bind.sessionGetCommonSync(
        sessionId: sessionId, key: 'is_screenshot_supported', param: '');
    if (isScreenshotSupported != 'true') return false;
    if (_manualThumbnailCapturePending) return false;

    final display = _pi.currentDisplay >= 0 ? _pi.currentDisplay : 0;
    final outputPath = await _thumbnailPathForPeer(peerId);
    _manualThumbnailOutputPath = outputPath;
    _manualThumbnailCapturePending = true;
    _manualThumbnailCaptureCompleter = Completer<bool>();
    bind.sessionTakeScreenshot(sessionId: sessionId, display: display);

    bool ok = false;
    try {
      ok = await _manualThumbnailCaptureCompleter!.future
  ...
  StreamEventHandler startEventListener(SessionID sessionId, String peerId) {
    return (evt) async {
      ...
      if (name == 'msgbox') {
        ...
        unawaited(_handlePrinterRequest(evt, sessionId, peerId));
  ...
  Future<void> _handleScreenshot(
    ...
    final msg = evt['msg'] ?? '';
    if (_manualThumbnailCapturePending) {
      bool ok = false;
      try {
        if (msg.isEmpty && _manualThumbnailOutputPath.isNotEmpty) {
          ok = await _saveScreenshotToPeerPreview(
              sessionId, peerId, _manualThumbnailOutputPath);
        } else {
          await bind.sessionHandleScreenshot(sessionId: sessionId, action: '2');
        }
      } finally {
        if (!(_manualThumbnailCaptureCompleter?.isCompleted ?? true)) {
          _manualThumbnailCaptureCompleter?.complete(ok);
        }
      }
      return;
    }
    if (_autoPreviewCapturePending) {
      _autoPreviewCapturePending = false;
      ...
      final outputPath = p.join(homeDir, 'SeeDesktop_preview_$peerId.png');
      await _saveScreenshotToPeerPreview(sessionId, peerId, outputPath);
      return;
    }
  ...
  Future<void> _handlePrinterRequest(
      Map<String, dynamic> evt, SessionID sessionId, String peerId) async {
    final id = evt['id'];
    final path = evt['path'];
    debugPrint(
        '[printer] Flutter received printer_request: sessionId=$sessionId, peerId=$peerId, id=$id, path=$path');
    if (!await hasProLicenseLocal()) {
      final ctx = globalKey.currentContext;
      if (ctx != null) {
        await showPremiumPaywallDialog(ctx);
      } else {
        showToast(
            'Premium feature: An active Pro license is required for remote printing.');
      }
      // Hard-stop free users before opening the printer dialog or starting print.
      return;
    }
    final dialogManager = parent.target!.dialogManager;
  ...
  handleMsgBox(Map<String, dynamic> evt, SessionID sessionId, String peerId) {
    ...
    if (type == 're-input-password') {
      ...
      // Silent automatic relay when direct connection fails (no user prompt).
  ...
  handlePeerInfo(Map<String, dynamic> evt, String peerId, bool isCache) async {
    ...
    if (connType == ConnType.defaultConn) {
      // Force control mode by default for desktop sessions.
      // If previous sessions left "View Mode" enabled, disable it on connect.
      final wasViewOnly = bind.sessionGetToggleOptionSync(
          sessionId: sessionId, arg: kOptionToggleViewOnly);
      if ((isDesktop || isWebDesktop) && wasViewOnly) {
      ...
      // Use Input source 2 as the desktop default to avoid global keyboard
      // grabbing issues on the local machine.
      if ((isDesktop || isWebDesktop) &&
          stateGlobal.getInputSource() != 'Input source 2') {
        await stateGlobal.setInputSource(sessionId, 'Input source 2');
      }
    }
  ...
  tryUseAllMyDisplaysForTheRemoteSession(String peerId) async {
    ...
    // Free/unlicensed can use only a single display window.
    if (!await hasProLicenseLocal()) {
      if (length > 1) {
        showToast(
            'Premium feature: An active Pro license is required for multi-display.');
      }
      return;
    }
...
class ViewStyle {
  ...
  double get scale {
    ...
    } else if (style == kRemoteViewStyleStretch) {
      if (width != 0 &&
          height != 0 &&
          displayWidth != 0 &&
          displayHeight != 0) {
        // Fill canvas fully; one axis may be cropped to remove borders.
        final s1 = width / displayWidth;
        final s2 = height / displayHeight;
        s = s1 > s2 ? s1 : s2;
      }
...
class FFI {
  ...
  void onEvent2UIRgba() async {
    ...
    if (ffiModel.waitForFirstImage.value == true) {
      ...
      unawaited(ffiModel.capturePeerDesktopPreviewIfNeeded(id));
  ...
  Future<void> close({bool closeSession = true}) async {
    closed = true;
    // Ensure freemium local slot is released immediately on any session close/drop.
