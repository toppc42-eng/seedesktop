...
/// After full restore, local `SeeDesktop_ab` still has folder/tag assignments, but the next
/// `pullAb` replaces them with the API. Merge disk snapshot into fetched lists, then sync to API.
bool _mergeModernAbFromDiskRestore(
  String diskJson,
  String profileGuid,
  List<Peer> tmpPeers,
  List<AbTag> tmpTags,
  ...
  try {
    ...
    final entries = data['ab_entries'];
    if (entries is! List) return false;
    Map<String, dynamic>? entry;
    for (final e in entries) {
      if (e is! Map<String, dynamic>) continue;
      final g = e['guid']?.toString() ?? '';
      if (g == profileGuid) {
        entry = e;
        break;
      }
      ...
      for (final p in peersList) {
        if (p is Map && p['id'] != null) {
          ...
          if (t is List && t.isNotEmpty) {
            idToTags[id] = t.map((e) => e.toString()).toList();
          }
        }
      }
    }
    for (final peer in tmpPeers) {
      final dt = idToTags[peer.id];
      if (dt == null || dt.isEmpty) continue;
      peer.tags = dt;
      changed = true;
    }

    final existingTagNames = tmpTags.map((e) => e.name).toSet();
    final diskTagList = entry['tags'];
    if (diskTagList is List) {
      for (final tn in diskTagList) {
        final name = tn.toString();
        if (existingTagNames.contains(name)) continue;
        int color =
            str2color2(name, existing: tmpTags.map((t) => t.color).toList())
                .value;
        if (entry['tag_colors'] is String) {
          try {
            final mc = jsonDecode(entry['tag_colors'] as String);
            if (mc is Map && mc[name] != null) {
              final v = mc[name];
              color = v is int ? v : int.tryParse(v.toString()) ?? color;
            }
          } catch (_) {}
        }
        tmpTags.add(AbTag(name, color));
        existingTagNames.add(name);
        changed = true;
      }
    }
  } catch (e) {
    debugPrint('_mergeModernAbFromDiskRestore: $e');
  }
  return changed;
}

bool _mergeLegacyAbFromDiskRestore(String diskJson, LegacyAb ab) {
  bool changed = false;
  try {
    final data = jsonDecode(diskJson);
    if (data is! Map) return false;
    ...
    Map<String, dynamic>? entry;
    for (final e in entries) {
      if (e is Map<String, dynamic> && e['name'] == _legacyAddressBookName) {
        entry = e;
        break;
      }
    }
    ...
    final diskTags = entry['tags'];
    if (diskTags is List) {
      for (final tn in diskTags) {
        final name = tn.toString();
        if (!ab.tags.contains(name)) {
          ab.tags.add(name);
          changed = true;
        }
      }
      ...
      try {
        ...
        if (mc is Map) {
          mc.forEach((k, v) {
            ...
            if (val != null && ab.tagColors[key] == null) {
              ab.tagColors[key] = val;
              changed = true;
            }
          });
        }
    ...
    if (changed) {
      ab.peers.refresh();
      ab.tags.refresh();
      ab.tagColors.refresh();
    }
  } catch (e) {
    debugPrint('_mergeLegacyAbFromDiskRestore: $e');
  }
  return changed;
}
...
class AbModel {
  ...
  reset() async {
    if (kDebugMode) debugPrint("reset ab model");
    addressbooks.clear();
    _currentName.value = '';
...
// #region tools
  ...
  Color getCurrentAbTagColor(String tag) {
    ...
    final colorValue = current.tagColors[tag];
    if (colorValue != null) {
...
abstract class BaseAb {
  ...
  removeNonExistentTags(Map<String, dynamic> p) {
    try {
      ...
      if (kDebugMode) debugPrint("removeNonExistentTags: $e");
    }
  }
...
class LegacyAb extends BaseAb {
  ...
  Future<bool> pullAbImpl({quiet = false}) async {
    ...
    if (ret && bind.mainAbRestoreMergePending()) {
      try {
        final disk = await bind.mainLoadAb();
        if (_mergeLegacyAbFromDiskRestore(disk, this)) {
          final ok = await pushAb(toastIfSucc: false, toastIfFail: false);
          if (ok) {
            await bind.mainClearAbRestoreMergePending();
          } else if (!quiet) {
            BotToast.showText(
                contentColor: Colors.orange,
                text: translate('push_ab_failed_tip'));
          }
        } else {
          await bind.mainClearAbRestoreMergePending();
        }
      } catch (e) {
        debugPrint('legacy restore merge: $e');
        await bind.mainClearAbRestoreMergePending();
      }
    }
    return ret;
...
class Ab extends BaseAb {
  ...
  Future<bool> pullAbImpl({quiet = false}) async {
    bool ret = true;
    final restoreMerge = bind.mainAbRestoreMergePending();
    String? diskSnapshot;
    if (restoreMerge) {
      diskSnapshot = await bind.mainLoadAb();
    }
    List<Peer> tmpPeers = [];
    ...
    final apiTagsSnapshot =
        List<AbTag>.from(tmpTags.map((t) => AbTag(t.name, t.color)));
    bool merged = false;
    if (restoreMerge && diskSnapshot != null && diskSnapshot.isNotEmpty) {
      merged = _mergeModernAbFromDiskRestore(
        diskSnapshot,
        profile.guid,
        tmpPeers,
        tmpTags,
    ...
    if (restoreMerge) {
      if (merged) {
        final ok = await _pushMergedTagsAfterRestore(
            apiTagsSnapshot, tmpPeers, tmpTags);
        if (ok) {
          await bind.mainClearAbRestoreMergePending();
        } else if (!quiet) {
          BotToast.showText(
  ...
  Future<bool> _pushMergedTagsAfterRestore(
    List<AbTag> tagsBeforeMerge,
    List<Peer> mergedPeers,
    List<AbTag> mergedTags,
  ) async {
    final before = {for (var t in tagsBeforeMerge) t.name: t};
    try {
      for (final t in mergedTags) {
        if (!before.containsKey(t.name)) {
          final ok = await addTags([t.name], {t.name: t.color});
          if (!ok) {
            return false;
          }
        }
      }
      for (final p in mergedPeers) {
        final ok = await changeTagForPeers([p.id], p.tags);
        if (!ok) {
          return false;
        }
      }
      ...
      debugPrint('_pushMergedTagsAfterRestore: $e');
      return false;
    }
  }
