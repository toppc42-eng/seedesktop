...
class _RemoteToolbarState extends State<RemoteToolbar> {
  ...
  Widget _buildToolbar(BuildContext context) {
    ...
    toolbarItems.add(const _LicenseScopeBadge());
...
class _LicenseScopeBadge extends StatefulWidget {
  const _LicenseScopeBadge();

  @override
  State<_LicenseScopeBadge> createState() => _LicenseScopeBadgeState();
}

class _LicenseScopeBadgeState extends State<_LicenseScopeBadge> {
  LocalLicenseTier _tier = LocalLicenseTier.unlicensed;
  Timer? _timer;

  String _label(LocalLicenseTier tier) {
    switch (tier) {
  ...
  Color _background(LocalLicenseTier tier) {
    switch (tier) {
      case LocalLicenseTier.proActive:
  ...
  void initState() {
    super.initState();
    _refreshTier();
    _timer = Timer.periodic(const Duration(seconds: 1), (_) => _refreshTier());
  }
  ...
  @override
  Widget build(BuildContext context) {
    final text = 'License: ${_label(_tier)}';
    return Container(
      margin: const EdgeInsets.symmetric(horizontal: 6),
      padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
      decoration: BoxDecoration(
        color: _background(_tier),
...
class _MonitorMenu extends StatelessWidget {
  ...
  List<Widget> buildMonitorList(BuildContext context, bool isMulti) {
    ...
    buildMonitorButton(int i) => Obx(() {
          ...
          return _IconMenuButton(
            ...
            onPressed: () => onPressed(context, i, pi, isMulti),
          );
        });
  ...
  Future<void> onPressed(
      BuildContext context, int i, PeerInfo pi, bool isMulti) async {
    final selectingExtraDisplay = i == kAllDisplayValue || i > 0;
    if (selectingExtraDisplay && !await hasProLicenseLocal()) {
