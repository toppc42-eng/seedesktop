...
class _DesktopTabState extends State<DesktopTab>
  ...
  void onWindowMaximize() {
    ...
    if (tabType == DesktopTabType.main) {
      stateGlobal.compactHomeConnectionLayout.value = false;
    }
    _setMaximized(true);
  ...
  void onWindowUnmaximize() {
    ...
    _setMaximized(false);
    if (tabType == DesktopTabType.main) {
      Future.microtask(() => _applyHomeCompactAfterUnmaximize());
    }
    super.onWindowUnmaximize();
  }

  /// Restore from maximized on the main home tab: center a small window and
  /// show only the two connection panels (handled in [ConnectionPage]).
  Future<void> _applyHomeCompactAfterUnmaximize() async {
    if (!isDesktop || tabType != DesktopTabType.main || bind.isIncomingOnly()) {
      return;
    }
    try {
      await Future.delayed(const Duration(milliseconds: 50));
      final tabs = controller.state.value.tabs;
      final sel = controller.state.value.selected;
      if (sel < 0 || sel >= tabs.length) {
        stateGlobal.compactHomeConnectionLayout.value = false;
        return;
      }
      if (tabs[sel].key != kTabLabelHomePage) {
        stateGlobal.compactHomeConnectionLayout.value = false;
        return;
      }
      stateGlobal.compactHomeConnectionLayout.value = true;
      const w = 600.0;
      const h = 430.0;
      await windowManager.setSize(const Size(w, h));
      await windowManager.center();
    } catch (e) {
      debugPrint('_applyHomeCompactAfterUnmaximize: $e');
    }
  }
  ...
  void onWindowClose() async {
    ...
    unawaited(_saveFrame(flush: true));
                    ...
                    Offstage(
                      ...
                      child: Row(children: [
                        Offstage(
                          ...
                          child: Image(
                            image:
                                const AssetImage('assets/see-desktop-tray.png'),
                            width: 32,
                            height: 32,
                            fit: BoxFit.contain,
                          ),
...
class _TabState extends State<_Tab> with RestorationMixin {
  ...
  bool get _showLicenseBadge =>
      widget.tabType == DesktopTabType.remoteScreen ||
      widget.tabType == DesktopTabType.fileTransfer ||
      widget.tabType == DesktopTabType.viewCamera ||
      widget.tabType == DesktopTabType.portForward ||
      widget.tabType == DesktopTabType.terminal;
      ...
      if (widget.tabBuilder == null) {
        return Row(
          ...
          children: [
            icon,
            labelWidget,
            if (_showLicenseBadge) ...[
              const SizedBox(width: 6),
              const _TabLicenseBadge(),
            ],
          ],
        );
      } else {
...
class _TabLicenseBadge extends StatelessWidget {
  const _TabLicenseBadge();

  @override
  Widget build(BuildContext context) {
    return FutureBuilder<LocalLicenseTier>(
      future: getLocalLicenseTier(),
      builder: (context, snapshot) {
        final tier = snapshot.data ?? LocalLicenseTier.unlicensed;
        final label = localLicenseTierShortLabel(tier);
        return Container(
          padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
          decoration: BoxDecoration(
            color: MyTheme.tabbar(context).selectedTabBackgroundColor,
            border: Border.all(color: MyTheme.border.withOpacity(0.35)),
