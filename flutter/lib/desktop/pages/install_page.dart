...
class _InstallPageBodyState extends State<_InstallPageBody>
  ...
  bool _autoInstallTriggered = false;
  ...
  void initState() {
    ...
    super.initState();
    // Auto-start installation when opening the installer page.
    Future.microtask(() {
      if (!mounted || _autoInstallTriggered || !btnEnabled.value) return;
      _autoInstallTriggered = true;
      install();
    });
  }
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  Text(
                    'Installed for all users in system path',
              ...
              Option(desktopicon, label: 'Create desktop icon')
                  .marginOnly(bottom: 7),
              Option(printer, label: 'Install Compatible Remote Printer'),
              Container(
  ...
  void install() {
    do_install() async {
      ...
      // Keep system service enabled so remote access works
      // before user logon and for any logged-in account.
      await bind.mainSetOption(key: kOptionStopService, value: '');
      String args = '';
      if (startmenu.value) args += ' startmenu';
      if (desktopicon.value) args += ' desktopicon';
      if (printer.value) args += ' printer';
      final forcedInstallPath = bind.installInstallPath();
      controller.text = forcedInstallPath;
      bind.installInstallMe(options: args, path: forcedInstallPath);
    }
