...
import '../../consts.dart';
import '../../common/widgets/crm_widgets.dart';
...
class RemotePage extends StatefulWidget {
  ...
  Future<void> requestCloseSession({
    FutureOr<void> Function()? onCloseSession,
      ...
      (_lastState.value! as _RemotePageState)
          .requestCloseSession(onCloseSession: onCloseSession);

  /// App exit: skip CRM UI and license release HTTP (runs release in background).
  Future<void> closeSessionForAppExit({
    FutureOr<void> Function()? onCloseSession,
  }) =>
      (_lastState.value! as _RemotePageState)
          .closeSessionForAppExit(onCloseSession: onCloseSession);
...
class _RemotePageState extends State<RemotePage>
  ...
  DateTime? _crmSessionStartTime;
  bool _crmCloseFlowActive = false;
  ...
  void initState() {
    super.initState();
    _bootstrapRemoteSession();
  }

  void _bootstrapRemoteSession() {
    _ffi = FFI(widget.sessionId);
    Get.put<FFI>(_ffi, tag: widget.id);
    _ffi.imageModel.addCallbackOnFirstImage((String peerId) {
      _crmSessionStartTime ??= DateTime.now();
      _ffi.canvasModel.activateLocalCursor();
      showKBLayoutTypeChooserIfNeeded(
          _ffi.ffiModel.pi.platform, _ffi.dialogManager);
      _ffi.recordingModel
          .updateStatus(bind.sessionGetIsRecording(sessionId: _ffi.sessionId));
      unawaited(_enforceUnlicensedClipboardRestriction());
      _startFreeSessionLimitTimerIfNeeded();
      reportEstablishedRemoteSessionToVps(
        targetPc: _licenseTargetPcLabelForVps(),
        peerId: widget.id,
        remoteHostname: _ffi.ffiModel.pi.hostname,
        remoteUsername: _ffi.ffiModel.pi.username,
      );
    });
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      // If the user cancels the pre-authorization "Connecting..." overlay,
      // immediately fire release_connection to the license/VPS server so the
      // dashboard does not keep a ghost active session for this peer. The
      // RemotePage.dispose teardown still runs after closeConnection(), but
      // we cannot rely on it alone because closeSession may end up false
      // (e.g. window-level close) and the release would be skipped.
      _ffi.dialogManager.showLoading(translate('Connecting...'), onCancel: () {
        unawaited(releaseConnectionForPeer(
          widget.id,
  ...
  /// Label for license / VPS session reporting (matches previous start_session target metadata).
  String _licenseTargetPcLabelForVps() {
    final alias =
        bind.mainGetPeerOptionSync(id: widget.id, key: 'alias').trim();
    var targetName = getDesktopTabLabel(widget.id, alias).trim();
    if (targetName.isEmpty || targetName.startsWith('{')) {
      targetName = alias.isEmpty ? widget.id : '$alias (${widget.id})';
    } else if (!targetName.contains(widget.id)) {
      targetName = '$targetName (${widget.id})';
    }
    return targetName;
  }
  ...
  Future<void> dispose() async {
    ...
    final crmStartTime = _crmSessionStartTime;
    final crmEndTime = DateTime.now();
    final crmPeerName = _licenseTargetPcLabelForVps();

    // Cancel timers immediately so callbacks cannot run during/after teardown
    // (dispose is async — a pending 30‑min unlicensed limit timer could otherwise
    // fire while awaiting release/close and call closeConnection / showDialog).
    _timer?.cancel();
    _timer = null;
    ...
    if (closeSession && crmStartTime != null) {
      _handleCrmSessionClosed(
        startTime: crmStartTime,
        endTime: crmEndTime,
  ...
  void _handleCrmSessionClosed({
    ...
    unawaited(CrmService.addLog(
      ...
      debugPrint('CRM addLog failed for ${widget.id}: $e');
    }));
  }

  /// Step 1: terminate network/session immediately (remote peer sees disconnect).
  Future<void> _disconnectNetworkImmediately() async {
    closeSessionOnDispose[widget.id] = false;
    unawaited(releaseConnectionForPeer(
      widget.id,
      sessionIdHint: sessionId.toString(),
    ));
  ...
  /// Step 5: close remote tab / screen after logging dialog (or skip dialog).
  Future<void> _popRemoteScreenAfterDisconnect({
    FutureOr<void> Function()? onCloseSession,
  }) async {
  ...
  Future<bool> _shouldShowCrmNoteDialog() async {
    if (_crmSessionStartTime == null) return false;
    if (!await hasProLicenseLocal()) return false;
    if (CloudSyncService.savedEmail.trim().isEmpty) return false;
    return true;
  }

  /// Steps 3–4: dialog on the remote screen (same [context]), then pop when done.
  Future<void> _showCrmNoteThenPopScreen({
    required DateTime endTime,
    FutureOr<void> Function()? onCloseSession,
  }) async {
    if (!mounted) {
    ...
    await showCrmDisconnectedNoteFlowPostClose(
      context: context,
      peerId: widget.id,
      endTime: endTime,
      onCloseSession: () =>
          _popRemoteScreenAfterDisconnect(onCloseSession: onCloseSession),
  ...
  Future<void> closeSessionForAppExit({
    ...
    if (_crmCloseFlowActive) return;
    _crmCloseFlowActive = true;
    try {
      final crmStart = _crmSessionStartTime;
      final endTime = DateTime.now();
      await _disconnectNetworkImmediately();
      if (crmStart != null) {
        _handleCrmSessionClosed(
          startTime: crmStart,
          endTime: endTime,
          peerName: _licenseTargetPcLabelForVps(),
        );
      }
      await _popRemoteScreenAfterDisconnect(onCloseSession: onCloseSession);
    } finally {
  ...
  /// Toolbar close, tab close, and system back — all use this flow.
  Future<void> requestCloseSession({
    FutureOr<void> Function()? onCloseSession,
  }) async {
    if (_crmCloseFlowActive) return;
    ...
    final startTime = _crmSessionStartTime;
    final endTime = DateTime.now();
    final peerName = _licenseTargetPcLabelForVps();
    ...
    try {
      ...
      if (await _shouldShowCrmNoteDialog()) {
        await _showCrmNoteThenPopScreen(
          endTime: endTime,
  ...
  void enterView(PointerEnterEvent evt) {
    final isStretch =
        _ffi.canvasModel.viewStyle.style == kRemoteViewStyleStretch;
    if (!isStretch) {
  ...
  Future<void> _startFreeSessionLimitTimerIfNeeded() async {
    ...
    if (!await shouldEnforceThirtyMinuteTimeoutLocal()) {
      return;
    }
  ...
  Future<void> _enforceUnlicensedClipboardRestriction() async {
    if (await hasProLicenseLocal()) {
      return;
    }
    final enabled = bind.sessionGetToggleOptionSync(
        sessionId: _ffi.sessionId, arg: kOptionEnableFileCopyPaste);
    if (enabled) {
      await bind.sessionToggleOption(
          sessionId: _ffi.sessionId, value: kOptionEnableFileCopyPaste);
    }
  }
}
...
class _ImagePaintState extends State<ImagePaint> {
  ...
  Widget build(BuildContext context) {
    ...
    bool isViewStretch() => c.viewStyle.style == kRemoteViewStyleStretch;
    ...
    if (!isViewStretch() &&
      ...
      if (isViewStretch() && c.size.width > 0 && c.size.height > 0) {
        final nativeSize = Size(
            c.getDisplayWidth().toDouble(), c.getDisplayHeight().toDouble());
        final stretchPaint = m.useTextureRender ||
                widget.ffi.ffiModel.pi.forceTextureRender
            ? _BuildPaintTextureRender(c, 1.0, Offset.zero, nativeSize, false)
            : _buildScrollbarNonTextureRender(m, nativeSize, 1.0);
        final stretchWidget = SizedBox.expand(
          child: FittedBox(
            fit: BoxFit.fill,
            child: SizedBox(
        ...
        return mouseRegion(child: _buildListener(stretchWidget));
      }
