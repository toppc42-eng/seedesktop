...
import 'package:flutter_hbb/utils/license_manager.dart'
    show normalizePeerIdForLicenseTracking;
...
class RustDeskMultiWindowManager {
  ...
  Future<MultiWindowCallResult> newSession(
    ...
    // Enforce tab-based sessions instead of opening new windows.
  ...
  Future<List<RemoteWindowCoords>> getOtherRemoteWindowCoordsFromMain() async {
    ...
    final dynamic res = await DesktopMultiWindow.invokeMethod(
        kMainWindowId, kWindowEventRemoteWindowCoords, kWindowId.toString());
    if (res == null) return coords;
    final raw = res is String ? res : jsonEncode(res);
    if (raw.trim().isEmpty) return coords;
    try {
      final decoded = jsonDecode(raw);
      if (decoded is! List) return coords;
      for (final item in decoded) {
        try {
          if (item is Map<String, dynamic>) {
            coords.add(RemoteWindowCoords.fromJson(item));
          } else if (item is String && item.trim().isNotEmpty) {
            final itemDecoded = jsonDecode(item);
            if (itemDecoded is Map<String, dynamic>) {
              coords.add(RemoteWindowCoords.fromJson(itemDecoded));
            }
          }
        } catch (_) {
          // Skip invalid item and continue parsing others.
        }
      }
    } catch (_) {
      // Ignore malformed payload and return empty coords.
    }
    return coords;
  }

  /// Normalized peer ids that currently have remote-related UI (all subwindows + main tabs).
  Future<Set<String>> collectLiveLicenseRelevantPeerIds() async {
    final out = <String>{};
    Future<void> scan(List<int> windowIds) async {
      for (final wId in windowIds) {
        try {
          final raw = await DesktopMultiWindow.invokeMethod(
              wId, kWindowEventGetRemoteList, null);
          if (raw is! String || raw.isEmpty) continue;
          for (final part in raw.split(',')) {
            final k = normalizePeerIdForLicenseTracking(part);
    ...
    await scan(_remoteDesktopWindows);
    await scan(_viewCameraWindows);
    await scan(_fileTransferWindows);
    await scan(_portForwardWindows);
    await scan(_terminalWindows);

    try {
      final rawMain = await DesktopMultiWindow.invokeMethod(
          kMainWindowId, kWindowEventGetRemoteList, null);
      if (rawMain is String && rawMain.isNotEmpty) {
        for (final part in rawMain.split(',')) {
          final k = normalizePeerIdForLicenseTracking(part);
          if (k.isNotEmpty) out.add(k);
        }
      }
