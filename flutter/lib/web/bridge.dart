...
class RustdeskImpl {
  ...
  Future<void> sessionSetImageQuality(
      {required UuidValue sessionId, required String value, dynamic hint}) {
    if (kDebugMode) debugPrint('set image quality: $value');
    return Future(
  ...
  String mainGetSysMetrics({dynamic hint}) {
    return jsonEncode({
      'cpu': -1,
      'ram_used_gb': 0,
      'ram_total_gb': 0,
      'disk_free_gb': 0,
      'disk_total_gb': 0,
      'hw_health': <String, dynamic>{},
    });
  }

  Future<void> executeSystemCommand({required String command, dynamic hint}) async {}
  ...
  Future<void> mainKillOtherInstancesSameExecutable({dynamic hint}) {
    return Future.value();
  }
  ...
  bool mainAbRestoreMergePending({dynamic hint}) => false;

  Future<void> mainClearAbRestoreMergePending({dynamic hint}) =>
      Future.value();
