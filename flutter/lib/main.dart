...
import 'package:flutter_hbb/utils/agent_task_handler.dart';
import 'package:flutter_hbb/utils/idle_heartbeat_service.dart';
...
Future<void> main(List<String> args) async {
  ...
  if (args.isNotEmpty && args.first == 'multi_window') {
    ...
    runInstallPage();
  } else if (args.contains(OtaWindowsScheduledTask.argSilent)) {
    await initEnv(kAppTypeMain);
    await OtaUpdateService.runScheduledSilentInstall();
    exit(0);
  } else {
...
Future<void> initEnv(String appType) async {
  ...
  await loadRmmScriptsUiPref();
}

Future<void> _applyDefaultLanguageByCountryIfUnset() async {
  final current = bind.mainGetLocalOption(key: kCommConfKeyLang).trim();
  if (current.isNotEmpty) return;

  final rawLocale = localeName.trim();
  final normalized = rawLocale.toUpperCase().replaceAll('-', '_');
  final isIsrael = normalized.contains('_IL');
  final targetLang = isIsrael ? 'he' : 'en';

  await bind.mainSetLocalOption(key: kCommConfKeyLang, value: targetLang);
  if (kDebugMode) {
    debugPrint(
      '[LangDefault] locale="$rawLocale" -> default lang "$targetLang"',
    );
  }
}

/// Post-update same-exe cleanup is disabled in Rust (`kill_other_instances_same_executable`
/// no-op) to avoid service/tray instability. Kept as a stub for future use.
Future<void> takeoverKillStaleSameExeInstances() async {
  if (!isDesktop) return;
}
...
void runMainApp(bool startService) async {
  // register uni links
  await initEnv(kAppTypeMain);
  await _applyDefaultLanguageByCountryIfUnset();
  await takeoverKillStaleSameExeInstances();
  checkUpdate();
  if (isWindows) {
  ...
  await cacheHardwareId(hardwareId);
  await performStartupSessionCleanup(hardwareId: hardwareId);
  await refreshLicenseStatusFromServer();
  await LicenseHeartbeatManager.instance.start();
  IdleHeartbeatService.instance.start();
  await checkAndSubmitPendingLogs();
  AgentHeartbeatManager.instance.start();
  ...
  await loadUpgradeMarketingPref();
  if (isDesktop) {
    AdminCloudSyncHooks.onRmmScriptsUiVisible = setRmmScriptsUiVisible;
    AdminCloudSyncHooks.onUpgradeMarketingVisible = setUpgradeMarketingPref;
...
class _AppState extends State<App> with WidgetsBindingObserver {
  ...
  bool _licenseSyncInProgress = false;
  @override
  void initState() {
    super.initState();
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    if (state == AppLifecycleState.resumed) {
      unawaited(_refreshLicenseStatusOnLifecycle());
      IdleHeartbeatService.instance.start();
      return;
    }
    if (state == AppLifecycleState.detached) {
      _releaseRequested = true;
      unawaited(_refreshAndReleaseOnDetach());
    }
  }

  Future<void> _refreshLicenseStatusOnLifecycle() async {
    if (_licenseSyncInProgress) return;
    _licenseSyncInProgress = true;
    try {
      await refreshLicenseStatusFromServer();
    } finally {
      _licenseSyncInProgress = false;
    }
  }

  Future<void> _refreshAndReleaseOnDetach() async {
    try {
      await _refreshLicenseStatusOnLifecycle();
    } finally {
      await releaseConnectionFromPrefs();
    }
  }
    ...
    return RefreshWrapper(builder: (context) {
      return MultiProvider(
        ...
        child: GetMaterialApp(
          ...
          debugShowCheckedModeBanner: false,
          title: isWeb
              ? '$kBrandedAppName Web Client V2 (Preview)'
              : kBrandedAppName,
          theme: MyTheme.lightTheme,
          darkTheme: MyTheme.darkTheme,
