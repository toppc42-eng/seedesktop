...
final RxMap<String, bool> _safeModeRestartPendingByPeer = <String, bool>{}.obs;
...
List<TTextMenu> toolbarControls(BuildContext context, String id, FFI ffi) {
  ...
  if (isDefaultConn && isDesktop) {
    ...
    if (pi.platform == kPeerPlatformWindows &&
      ...
      v.add(
        TTextMenu(
          child: Text('PowerShell commands'),
          onPressed: () async {
            if (!await hasRmmLicenseLocal()) {
              if (context.mounted) await showRmmProGateDialog(context);
              return;
            }
            final peerKey = id.replaceAll(' ', '');
            final displayName =
                pi.hostname.isNotEmpty ? pi.hostname : peerKey;
            unawaited(showPsCommandsCatalogDialog(
              context,
  ...
  if (isDefaultConn &&
    ...
    if (pi.platform == kPeerPlatformWindows) {
      final safeModePending = _safeModeRestartPendingByPeer[id] ?? false;
      v.add(
        TTextMenu(
            child: Text(translate(safeModePending
                ? 'Restart remote device (Normal mode)'
                : 'Restart to Safe Mode (Pro)')),
            onPressed: () async {
              if (safeModePending) {
                final confirmed = await showRestartRemoteDevice(
                    pi, id, sessionId, ffi.dialogManager);
                if (confirmed) {
                  _safeModeRestartPendingByPeer[id] = false;
                }
                return;
              }

              final hasPro = await hasProOrRmmLicenseLocal();
              if (!hasPro) {
                await showPremiumPaywallDialog(context);
                return;
              }
              final confirmed = await showRestartRemoteDeviceSafeMode(
                  pi, id, sessionId, ffi.dialogManager);
              if (confirmed) {
                _safeModeRestartPendingByPeer[id] = true;
              }
            }),
      );
    }
  ...
  // Replace screenshot action with File Transfer entry.
  if (isDesktop && isDefaultConn) {
    v.add(TTextMenu(
      child: Text(translate('Transfer file')),
...
Future<List<TRadioMenu<String>>> toolbarViewStyle(
  ...
  return [
    ...
    TRadioMenu<String>(
        ...
        onChanged: onChanged),
    TRadioMenu<String>(
        child: Text(translate('Scale stretch')),
        value: kRemoteViewStyleStretch,
        groupValue: groupValue,
        onChanged: onChanged),
    TRadioMenu<String>(
  ...
  if (isDefaultConn &&
    ...
    v.add(TToggleMenu(
        ...
        onChanged: enabled
            ? (value) async {
                if (value == null) return;
                if (value && !await hasProOrRmmLicenseLocal()) {
                  await showPremiumPaywallDialog(context);
                  return;
                }
                bind.sessionToggleOption(
...
List<TToggleMenu> toolbarKeyboardToggles(FFI ffi) {
  ...
  // Sync Num Lock state to remote (persistent user default).
  if (ffiModel.keyboard) {
    final optionValue = bind.mainGetUserDefaultOption(key: kKeySyncNumLock);
    final value = optionValue != 'N';
    onChanged(bool? changed) {
      if (changed == null) return;
      bind.mainSetUserDefaultOption(
          key: kKeySyncNumLock, value: changed ? 'Y' : 'N');
      ffi.inputModel.setSyncNumLock(changed);
    }
