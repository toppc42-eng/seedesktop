...
import 'package:flutter_hbb/utils/license_manager.dart' show releaseConnectionForPeer;
...
/// Fires release_connection to the license/VPS server immediately when the
/// user aborts a remote-desktop attempt BEFORE a successful session establishes
/// (e.g. clicks Cancel on the password / login dialog or the "Connecting..."
/// overlay). Without this, the WordPress License Manager keeps a ghost active
/// session for the peer because the RemotePage.dispose release path may run
/// with closeSession=false or race the tab teardown across windows.
void _releaseLicenseSessionForCancel(String? peerId, {SessionID? sessionId}) {
...
void wrongPasswordDialog(SessionID sessionId,
  ...
  dialogManager.show((setState, close, context) {
    cancel() {
      close();
      _releaseLicenseSessionForCancel(peerId, sessionId: sessionId);
      closeConnection();
    }
...
_connectDialog(
  ...
  dialogManager.show((setState, close, context) {
    ...
    submit() {
      ...
      close();
      // If the user cancels the post-credentials "Logging in..." overlay
      // (server still hasn't authorized), treat it like the password dialog
      // cancel and release the VPS slot immediately.
      dialogManager.showLoading(translate('Logging in...'), onCancel: () {
        _releaseLicenseSessionForCancel(peerId, sessionId: sessionId);
        closeConnection();
      });
    }
...
Future<bool> showRestartRemoteDevice(PeerInfo pi, String id,
  ...
  if (res == true) {
    await bind.sessionPeerOption(
        sessionId: sessionId, name: 'restart-mode', value: '');
...
Future<bool> showRestartRemoteDeviceSafeMode(PeerInfo pi, String id,
    SessionID sessionId, OverlayDialogManager dialogManager) async {
  final res = await dialogManager
      .show<bool>((setState, close, context) => CustomAlertDialog(
            title: Row(children: [
              Icon(Icons.warning_rounded, color: Colors.redAccent, size: 28),
              Flexible(
                  child: Text(translate("Restart to Safe Mode (Network)"))
                      .paddingOnly(left: 10)),
            ]),
            content: Text(
                "${translate('Restart')} \n${pi.username}@${pi.hostname}($id)\n${translate('into Safe Mode with Networking?')}\n\n${translate('The remote service will be allowed in Safe Mode so you can reconnect.')}"),
            actions: [
              dialogButton(
                "Cancel",
                icon: Icon(Icons.close_rounded),
  ...
  if (res == true) {
    await bind.sessionPeerOption(
        sessionId: sessionId, name: 'restart-mode', value: 'safe');
    bind.sessionRestartRemoteDevice(sessionId: sessionId);
    return true;
  }
  return false;
...
void addPeersToAbDialog(
  ...
  if (!await hasProLicenseLocal()) {
    ...
    if (ctx != null) {
      ...
      showToast('Adding computers to Address Book requires an active license');
    }
    return;
  }
