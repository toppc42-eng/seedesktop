...
const val DEFAULT_NOTIFY_TITLE = "SeeDesktop"
...
class MainService : Service() {
    ...
    fun rustPointerInput(kind: Int, mask: Int, x: Int, y: Int) {
        ...
        if (!powerManager.isInteractive && (kind == 0 || mask == LEFT_DOWN)) {
            ...
            // Some clients may report touch kind while still using mouse masks.
            // Route by mask to avoid dropping click events on Android hosts.
            val isMouseMask = when (mask) {
                LEFT_MOVE, LEFT_DOWN, LEFT_UP, RIGHT_UP, BACK_UP,
                WHEEL_BUTTON_DOWN, WHEEL_BUTTON_UP, WHEEL_DOWN, WHEEL_UP -> true
                else -> false
            }
            when (kind) {
                0 -> { // touch
    ...
    private val mediaProjectionCallback = object : MediaProjection.Callback() {
        override fun onStop() {
            Log.w(logTag, "MediaProjection stopped by system/user")
            stopCapture()
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        if (intent?.action == ACT_INIT_MEDIA_PROJECTION_AND_SERVICE) {
            // Android 14 checks happen at getMediaProjection(), so enter FGS with
            // MEDIA_PROJECTION type first (and keep DATA_SYNC for legacy behavior).
            val fgsType = ServiceInfo.FOREGROUND_SERVICE_TYPE_MEDIA_PROJECTION or
                ServiceInfo.FOREGROUND_SERVICE_TYPE_DATA_SYNC
            createForegroundNotification(fgsType)
            ...
            intent.getParcelableExtra<Intent>(EXT_MEDIA_PROJECTION_RES_INTENT)?.let {
                Log.d(logTag, "media projection result intent received, acquiring token")
                mediaProjection?.unregisterCallback(mediaProjectionCallback)
                mediaProjection =
                    mediaProjectionManager.getMediaProjection(Activity.RESULT_OK, it)
                mediaProjection?.registerCallback(mediaProjectionCallback, serviceHandler)
                Log.d(logTag, "media projection token acquired successfully")
                checkMediaPermission()
                _isReady = true
            } ?: let {
    ...
    private fun initNotification() {
        ...
        notificationChannel = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channelId = "SeeDesktop"
            val channelName = "SeeDesktop Service"
            val channel = NotificationChannel(
                channelId,
                channelName, NotificationManager.IMPORTANCE_HIGH
            ).apply {
                description = "SeeDesktop Service Channel"
            }
    ...
    private fun createForegroundNotification(serviceType: Int? = null) {
