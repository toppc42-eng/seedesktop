...
class InputService : AccessibilityService() {
    ...
    fun onMouseInput(mask: Int, _x: Int, _y: Int) {
        val eventType = mask and MOUSE_TYPE_MASK
        if (eventType == MOUSE_TYPE_WHEEL) {
            performWheelScroll(_y)
            return
        }
    ...
    @RequiresApi(Build.VERSION_CODES.N)
    private fun performWheelScroll(deltaY: Int) {
        if (deltaY == 0) return
        val safeX = max(1, mouseX)
        val safeY = max(WHEEL_STEP + 1, mouseY)
        val path = Path()
        path.moveTo(safeX.toFloat(), safeY.toFloat())
        val targetY = if (deltaY < 0) {
            // Remote wheel-down should scroll content down (finger swipes up).
            max(1, safeY - WHEEL_STEP)
        } else {
            safeY + WHEEL_STEP
        }
        path.lineTo(safeX.toFloat(), targetY.toFloat())
