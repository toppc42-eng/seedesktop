...
pub fn is_rd_printer_installed(app_name: &str) -> ResultType<bool> {
    ...
    let seede_printer_name = PCWSTR::from_raw(custom_printer_name.as_ptr());
    Ok(printer::is_printer_added(&rd_printer_name)?
        || printer::is_printer_added(&seede_printer_name)?)
}
