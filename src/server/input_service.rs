...
// Ignore tiny absolute move oscillations that happen within a very short interval.
// This reduces visible mouse jitter in remote sessions without impacting normal movement.
const MOUSE_JITTER_DEADZONE_PX: i32 = 1;
const MOUSE_JITTER_MAX_GAP_MS: i64 = 80;
...
pub fn handle_mouse_(
    ...
    if should_ignore_mouse_jitter(evt, conn) {
        return;
    }
...
#[inline]
fn should_ignore_mouse_jitter(evt: &MouseEvent, conn: i32) -> bool {
    let evt_type = evt.mask & MOUSE_TYPE_MASK;
    if evt_type != MOUSE_TYPE_MOVE {
        return false;
    }
    // Never filter drag/pressed movement.
    if (evt.mask >> 3) != 0 {
        return false;
    }
    // Relative mode has dedicated delta semantics; don't apply absolute jitter filtering.
    if is_relative_mouse_active(conn) {
        return false;
    }

    let now = get_time();
    let last = *LATEST_PEER_INPUT_CURSOR.lock().unwrap();
    if last.conn != conn || last.time <= 0 {
        return false;
    }
    if now - last.time > MOUSE_JITTER_MAX_GAP_MS {
        return false;
    }

    (evt.x - last.x).abs() <= MOUSE_JITTER_DEADZONE_PX
        && (evt.y - last.y).abs() <= MOUSE_JITTER_DEADZONE_PX
}
