...
#[cfg(target_os = "windows")]
fn restart_windows_safe_mode_with_network() -> Result<(), String> {
    let app_name = crate::get_app_name();
    let safeboot_key = format!(
        "HKLM\\SYSTEM\\CurrentControlSet\\Control\\SafeBoot\\Network\\{}",
        app_name
    );

    let reg_status = Command::new("reg")
        .args(["add", &safeboot_key, "/ve", "/t", "REG_SZ", "/d", "Service", "/f"])
        .status()
        .map_err(|e| format!("failed to run reg add: {}", e))?;
    if !reg_status.success() {
        return Err(format!("reg add failed with status: {}", reg_status));
    }

    // Set current OS to boot into safe mode with networking on next restart
    let set_safe_status = Command::new("bcdedit")
        .args(["/set", "{current}", "safeboot", "network"])
        .status()
        .map_err(|e| format!("failed to run bcdedit /set safeboot: {}", e))?;
    if !set_safe_status.success() {
        return Err(format!(
            "bcdedit /set safeboot failed with status: {}",
            set_safe_status
        ));
    }

    let shutdown_status = Command::new("shutdown")
        .args(["/r", "/t", "0", "/f"])
        .status()
        .map_err(|e| format!("failed to run shutdown: {}", e))?;
    if !shutdown_status.success() {
        return Err(format!("shutdown failed with status: {}", shutdown_status));
    }
    Ok(())
}

#[cfg(target_os = "windows")]
fn restart_windows_normal_mode() -> Result<(), String> {
    // Remove safeboot flag from current OS
    let _ = Command::new("bcdedit")
        .args(["/deletevalue", "{current}", "safeboot"])
        .status();
    let _ = Command::new("bcdedit")
        .args(["/deletevalue", "{current}", "safebootalternateshell"])
        .status();
    
    // Also try to clean up any legacy "SeeDesktop Safe Mode" entries that might have been created
    // by older versions of the software.
    let _ = Command::new("cmd")
        .args(["/c", "for /f \"tokens=2 delims={}\" %a in ('bcdedit /enum ^| findstr /i \"SeeDesktop Safe Mode\"') do bcdedit /delete {%a} /f"])
        .status();
...
impl Connection {
                    ...
                    match data {
                        ...
                        ipc::Data::PrinterData(data) => {
                            if Self::permission(keys::OPTION_ENABLE_REMOTE_PRINTER, &conn.control_permissions) {
                                log::info!(
                                    "Forwarding printer payload to peer connection: conn_id={}, data_len={}",
                                    conn.inner.id(),
                                    data.len()
                                );
                                ...
                                log::warn!(
                                    "Remote printer payload blocked by permission: conn_id={}",
                                    conn.inner.id()
                                );
    ...
    async fn send_logon_response(&mut self) {
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            platform_additions.insert(
                "remote_wallpaper_station_number".into(),
                json!(Config::get_id()),
            );
            if let Some(b64) = crate::platform::windows::get_current_wallpaper_jpeg_base64(640, 60)
            {
                platform_additions.insert("remote_wallpaper_jpeg_b64".into(), json!(b64));
            }
        ...
        #[cfg(any(target_os = "windows", feature = "unix-file-copy-paste"))]
        {
            let is_both_windows = cfg!(target_os = "windows")
                && self
                    .lr
                    .my_platform
                    .eq_ignore_ascii_case(&hbb_common::whoami::Platform::Windows.to_string());
            #[cfg(feature = "unix-file-copy-paste")]
            ...
            let is_both_macos = cfg!(target_os = "macos")
                && self
                    .lr
                    .my_platform
                    .eq_ignore_ascii_case(&hbb_common::whoami::Platform::MacOS.to_string());
                        ...
                        terminal_service::is_service_specified_user(&self.terminal_service_id)
                    {
                        if let Some(user_token) = &self.terminal_user_token {
                            ...
                            if is_user != has_service_token {
                                // Migrate gracefully when receiving a stale terminal service id
                                // created in an older impersonation mode. We reopen under a new
                                // SYSTEM-mode service id instead of failing the login.
                                log::warn!(
                                    "Terminal service token mode mismatch for {}, regenerating terminal service id",
                                    self.terminal_service_id
                                );
            ...
            match msg.union {
                            ...
                            Some(file_action::Union::Send(s)) => {
                                ...
                                match job_type {
                                    ...
                                    JobType::Printer => {
                                        ...
                                        {
                                            log::info!(
                                                "Matched printer payload by path for send-confirm: conn_id={}, path={}, data_len={}",
                                                self.inner.id(),
                                                path,
                                                data.len()
                                            );
                                            ...
                                            log::error!(
                                                "Printer payload not found for send-confirm: conn_id={}, path={}",
                                                self.inner.id(),
                                                path
                                            );
                                            return true;
                ...
                Some(message::Union::Misc(misc)) => match misc.union {
                    ...
                    Some(misc::Union::RestartRemoteDevice(_)) => {
                        ...
                        if self.restart {
                            ...
                            #[cfg(any(target_os = "linux", target_os = "windows"))]
                            {
                                #[cfg(target_os = "windows")]
                                {
                                    match restart_windows_normal_mode() {
                                        Ok(_) => log::info!("Restart normal mode by the peer"),
                                        Err(e) => log::error!("Failed to restart normal mode: {}", e),
                                    }
                                }
                                #[cfg(target_os = "linux")]
                                {
                    ...
                    Some(misc::Union::RestartRemoteDeviceSafeMode(_)) => {
                        if self.restart {
                            match restart_windows_safe_mode_with_network() {
                                Ok(_) => log::info!("Restart to Safe Mode by the peer"),
                                Err(e) => log::error!("Failed to restart into Safe Mode: {}", e),
                            }
                        }
                    }
                    #[cfg(windows)]
    ...
    // Windows terminal commands run as the current process user (service context / LocalSystem),
    ...
    async fn send_printer_request(&mut self, data: Vec<u8>) {
        ...
        log::info!(
            "Creating remote printer job request: conn_id={}, path={}, data_len={}",
            self.inner.id(),
            path,
            data.len()
        );
        ...
        log::info!(
            "Queued printer payload in memory for conn_id={}, pending_jobs={}",
            self.inner.id(),
            self.printer_data.len()
        );
    }
...
pub fn on_printer_data(data: Vec<u8>) {
    let conns = crate::server::AUTHED_CONNS.lock().unwrap();
    let eligible: Vec<_> = conns.iter().filter(|c| c.printer).collect();
    log::info!(
        "Received printer bytes from adapter: data_len={}, eligible_connections={}",
        data.len(),
        eligible.len()
    );
    let mut delivered = 0usize;
    for c in eligible {
        if c.sender.send(Data::PrinterData(data.clone())).is_ok() {
            delivered += 1;
            log::info!(
                "Dispatched printer payload to connection: conn_id={}",
                c.conn_id
            );
        } else {
            log::warn!(
                "Failed to send printer data to conn {}, trying next eligible connection",
                c.conn_id
            );
        }
    }
    if delivered == 0 {
        log::warn!("Printer data captured but no eligible active connection received it");
    } else {
        log::info!(
            "Printer payload dispatched successfully to {} connection(s)",
            delivered
        );
    }
}
...
impl Drop for Connection {
    fn drop(&mut self) {
        ...
        #[cfg(target_os = "windows")]
        let _ = self.terminal_user_token.take();
    }
}
