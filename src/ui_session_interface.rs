...
impl<T: InvokeUiSession> Session<T> {
    ...
    pub fn restart_remote_device(&self) {
        ...
        let restart_mode = lc.get_option("restart-mode");
        let msg = if restart_mode == "safe" {
            // Prevent stale mode from leaking into future restarts.
            lc.set_option("restart-mode".to_owned(), "".to_owned());
            lc.restart_remote_device_safe_mode()
        } else {
            lc.restart_remote_device()
        };
        self.send(Data::Message(msg));
    }
    ...
    pub fn printer_response(&self, id: i32, path: String, printer_name: String) {
        log::info!(
            "Printer response from UI: id={}, path={}, selected_printer='{}'",
            id,
            path,
            printer_name
        );
        ...
        log::info!(
            "Starting printer receive job: id={}, temp_path={}",
            id,
            to.to_string_lossy()
        );
