...
#define PRINT_XPS_CHECK_HR(hr, msg)                      \
    ...
    int PrintXPSRawData(LPWSTR printerName, BYTE *rawData, ULONG dataSize)
    {
        ...
        // For PDF/virtual printers, the Save As dialog often appears behind the app.
        // Spawn a helper thread to bring it to foreground.
        bool isPdfPrinter = false;
        {
            wchar_t nameLower[256] = {0};
            size_t len = wcsnlen_s(printerName, 255);
            for (size_t i = 0; i < len && i < 255; i++) {
                nameLower[i] = (wchar_t)towlower(printerName[i]);
            }
            isPdfPrinter = (wcsstr(nameLower, L"pdf") != nullptr);
        }
        if (isPdfPrinter) {
            std::thread([completionEvent]() {
                Sleep(1500);  // Let the Save As dialog appear
                for (int i = 0; i < 60; i++) {  // Try for ~30 seconds
                    if (WaitForSingleObject(completionEvent, 0) == WAIT_OBJECT_0)
                        break;
                    struct EnumCtx { HWND found = nullptr; };
                    EnumCtx ctx;
                    EnumWindows([](HWND hwnd, LPARAM lp) -> BOOL {
                        EnumCtx* c = (EnumCtx*)lp;
                        if (!IsWindowVisible(hwnd)) return TRUE;
                        wchar_t cls[32] = {0};
                        if (GetClassNameW(hwnd, cls, 31) && wcscmp(cls, L"#32770") == 0) {
                            wchar_t title[256] = {0};
                            GetWindowTextW(hwnd, title, 255);
                            std::wstring t(title);
                            for (auto& ch : t) ch = (wchar_t)towlower(ch);
                            if (t.find(L"save") != std::wstring::npos ||
                                t.find(L"print") != std::wstring::npos ||
                                t.find(L"pdf") != std::wstring::npos ||
                                t.find(L"\u05e9\u05de\u05d9\u05e8\u05d4") != std::wstring::npos) {  // Hebrew "save"
                                c->found = hwnd;
