...
async fn run_service(_arguments: Vec<OsString>) -> ResultType<()> {
    log::info!("Starting SeeDesktop service");
    // Create a marker file to indicate service is running updated code
    std::fs::write(r"C:\Windows\Temp\seedesktop_service_running.txt", "updated").ok();
    ...
    log::info!("Service: current session id {}", session_id);

    // Start printer capture in the SYSTEM service process so the XPS render filter
    // named pipe can be created with the required SYSTEM token. Captured print jobs
    // are forwarded to the user-session server process via IPC (Data::PrinterData).
    #[cfg(all(target_os = "windows", feature = "flutter"))]
    crate::server::printer_service::start_in_service();

    log::info!("Service: launching server process...");
    std::fs::write(
        r"C:\Windows\Temp\seedesktop_launch_server.txt",
    ...
    match launch_server(session_id, true).await {
        Ok(h_process) => {
            log::info!(
                "Service: server process launched successfully, handle: {:?}",
                h_process
            );
            std::fs::write(
                r"C:\Windows\Temp\seedesktop_launch_server.txt",
                format!("success: {:?}", h_process),
            )
            .ok();
        }
        Err(e) => {
            log::error!("Service: failed to launch server process: {}", e);
            std::fs::write(
                r"C:\Windows\Temp\seedesktop_launch_server.txt",
                format!("failed: {}", e),
            )
    ...
    let mut h_process = NULL; // For now, don't use the launched process
...
async fn launch_server(session_id: DWORD, close_first: bool) -> ResultType<HANDLE> {
    ...
    log::info!("Service: launching server from: {}", exe_path_str);
    let cmd = format!("\"{}\" --server", exe_path_str);
    log::info!("Service: launch command: {}", cmd);
...
if exist \"{tmp_path}\\{app_name} Tray.lnk\" del /f /q \"{tmp_path}\\{app_name} Tray.lnk\"
    ...
    // Network settings are force-locked in this build (server, api and key pinning).
...
")
    ...
    let install_remote_printer = if install_printer {
        ...
        // The script will not exit even if printer install panics.
        format!("\"{}\" --install-original-remote-printer", &src_exe)
        ...
        if metadata.is_file() {
            if !custom_client_staging_dir.exists() {
                if let Err(e) = std::fs::create_dir_all(custom_client_staging_dir) {
                    bail!(
                        "Failed to create parent directory {:?} when staging custom client files: {}",
                        custom_client_staging_dir,
                        e
                    );
                }
...
/// Text shown in services.msc under Description for the SeeDesktop Windows service.
const WINDOWS_SERVICE_DESCRIPTION: &str = "SeeDesktop remote desktop background service. Enables unattended remote access, screen sharing, and connection relay.";

fn get_set_service_description() -> String {
    format!(
        "sc description {app_name} \"{desc}\"",
        app_name = crate::get_app_name(),
        desc = WINDOWS_SERVICE_DESCRIPTION,
    )
}
...
",
    app_name = crate::get_app_name(),
    set_description = get_set_service_description())
    }
}
...
pub fn get_current_wallpaper_jpeg_base64(_max_edge: u32, quality: u8) -> Option<String> {
    let wallpaper_path = WallPaperRemover::get_recent_wallpaper()
        .ok()
        .filter(|p| !p.trim().is_empty())
        .or_else(|| wallpaper::get().ok().filter(|p| !p.trim().is_empty()))?;
    let path = std::path::Path::new(&wallpaper_path);
    if !path.exists() {
        return None;
    }

    let mut image = ImageReader::open(path).ok()?.decode().ok()?;
    let (w, h) = image.dimensions();
    if w == 0 || h == 0 {
        return None;
    }

    // Requirement: reduce linear dimensions by exactly 70% with interpolation.
    let mut new_w = ((w as f32) * 0.7).round() as u32;
    let mut new_h = ((h as f32) * 0.7).round() as u32;
    if new_w == 0 {
        new_w = 1;
    }
    if new_h == 0 {
    ...
    let target_bytes = 100 * 1024usize;
    let start_quality = quality.clamp(35, 90);
    for q in [start_quality, 50, 40, 35] {
        jpeg.clear();
        let mut encoder = JpegEncoder::new_with_quality(&mut jpeg, q);
...
fn get_uninstall_amyuni_idd() -> String {
    match std::env::current_exe() {
        ...
        Err(e) => {
            log::warn!(
                "Failed to get current exe path, cannot get command of uninstalling idd, Zzerror: {:?}",
                e
            );
            "".to_string()
        }
...
fn append_printer_diag_log(message: &str) {
...
pub fn send_raw_data_to_printer(printer_name: Option<String>, data: Vec<u8>) -> ResultType<()> {
    let normalize_name = |s: &str| s.trim_matches(char::from(0)).trim().to_string();
    let mut printer_name = normalize_name(&printer_name.unwrap_or_default());
    append_printer_diag_log(&format!(
        "send_raw_data_to_printer begin: requested_printer='{}', data_len={}",
        printer_name,
        data.len()
    ));
    let get_default_printer_name = || -> Option<String> {
        // GetDefaultPrinterW expects number of UTF-16 chars (including trailing '\0').
        let mut needed_chars = 0u32;
        unsafe {
            GetDefaultPrinterW(std::ptr::null_mut(), &mut needed_chars);
        }
        if needed_chars == 0 {
            return None;
        }
        let mut default_printer_name = vec![0u16; needed_chars as usize];
        let ok = unsafe {
            GetDefaultPrinterW(
        ...
        let len = default_printer_name
            .iter()
            .position(|&c| c == 0)
            .unwrap_or(default_printer_name.len());
        let name = String::from_utf16_lossy(&default_printer_name[..len]);
    ...
    if let Ok(names) = crate::platform::windows::get_printer_names() {
        if printer_name.is_empty() {
            if let Some(default_name) = get_default_printer_name() {
                printer_name = default_name;
            }
        } else {
            // Normalize and resolve selected printer with a case-insensitive match.
            if let Some(found) = names
                .iter()
                .find(|n| normalize_name(n).eq_ignore_ascii_case(&printer_name))
                .cloned()
            {
                printer_name = found;
            } else {
                log::warn!(
                    "Configured printer \"{}\" not found. Trying default printer fallback.",
                    printer_name
                );
                if let Some(default_name) = get_default_printer_name() {
                    printer_name = default_name;
                } else if let Some(first_name) = names.first() {
                    // Last-resort fallback to avoid dropping the print job completely.
                    printer_name = first_name.clone();
                    log::warn!(
                        "Default printer unavailable. Falling back to first installed printer: {}",
                        printer_name
                    );
                }
            }
    ...
    if printer_name.is_empty() {
        append_printer_diag_log("send_raw_data_to_printer failed: resolved printer name empty");
        return Err(anyhow!("Failed to get printer name"));
    }

    log::info!("Sending data to printer: {}", &printer_name);
    append_printer_diag_log(&format!(
        "send_raw_data_to_printer resolved printer='{}'",
        printer_name
    ));

    // Validate XPS format: XPS files are ZIP-based and start with "PK" (0x50 0x4B)
    if data.len() < 4 {
        append_printer_diag_log("WARNING: Print data too short to be valid XPS");
        log::warn!(
            "Print data too short ({} bytes), may be corrupted",
            data.len()
        );
    } else if data[0] != 0x50 || data[1] != 0x4B {
        append_printer_diag_log(&format!(
            "WARNING: Print data does not start with XPS/ZIP signature PK, got {:02X} {:02X}",
            data[0], data[1]
        ));
        log::warn!(
            "Print data may be corrupted: expected XPS (PK) signature, got {:02X} {:02X}",
            data[0],
            data[1]
        );
    }

    // Debug: save raw XPS to temp file when env var is set (for diagnosing corrupted output)
    if std::env::var("SEEDESKTOP_DEBUG_PRINT_SAVE_XPS").is_ok() {
        let temp = std::env::temp_dir().join("seedesktop_debug_print.xps");
        if let Some(temp_str) = temp.to_str() {
            if let Err(e) = std::fs::write(temp_str, &data) {
                log::warn!("Failed to save debug XPS: {}", e);
            } else {
                log::info!("Debug XPS saved to {}", temp_str);
                append_printer_diag_log(&format!("Debug XPS saved to {}", temp_str));
            }
        }
    }
    ...
    unsafe {
        ...
        if res != 0 {
            append_printer_diag_log(&format!(
                "PrintXPSRawData failed: code={}, see C:\\Windows\\temp\\test_rustdesk.log",
                res
            ));
            bail!(
                "Failed to send data to the printer, see logs in C:\\Windows\\temp\\test_rustdesk.log, %LOCALAPPDATA%\\SeeDesktop\\logs\\printer_bridge.log, and C:\\ProgramData\\SeeDesktop\\logs\\printer_bridge.log for details."
            );
        } else {
            log::info!("Successfully sent data to the printer");
            append_printer_diag_log("PrintXPSRawData success");
        }
    }
