...
pub async fn start_server(is_server: bool, no_server: bool) {
    ...
    if is_server {
        ...
        #[cfg(all(target_os = "windows", feature = "flutter"))]
        match printer_service::start() {
            Ok(()) => log::info!("Printer capture service started"),
            Err(e) => log::error!("Printer capture service failed to start: {}", e),
        }
        ...
        log::info!("Starting rendezvous mediator...");
        crate::RendezvousMediator::start_all().await;
        log::info!("Rendezvous mediator started");
    } else {
