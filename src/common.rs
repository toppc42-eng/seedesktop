...
pub const FORCED_RENDEZVOUS_SERVER: &str = "api.seedesktop.com";
pub const FORCED_RELAY_SERVER: &str = "api.seedesktop.com";
pub const FORCED_API_SERVER: &str = "https://api.seedesktop.com";
// Paste your id_ed25519.pub value here for strict server key pinning.
pub const FORCED_SERVER_KEY: &str = "KafXrl4RSnR62ccbm15Jk8kd7sBSYaW9JxSXbGw8224=";
...
pub async fn get_rendezvous_server(ms_timeout: u64) -> (String, Vec<String>, bool) {
    #[cfg(any(target_os = "android", target_os = "ios"))]
    let _ = get_rendezvous_server_(ms_timeout);
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    let _ = get_rendezvous_server_(ms_timeout).await;
    (
        socket_client::check_port(FORCED_RENDEZVOUS_SERVER, config::RENDEZVOUS_PORT),
        vec![],
        true,
    )
}
...
pub fn get_sysinfo() -> serde_json::Value {
    ...
    // Rich hardware snapshot (cpu_model, ram_gb, slots, disk_info, temps, …) for API / Flask.
    out["hw_health"] = crate::hardware_health::get_hardware_health();
    out
}
...
/// SeeDesktop does not call the public RustDesk update API.
#[tokio::main(flavor = "current_thread")]
