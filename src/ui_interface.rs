...
lazy_static::lazy_static! {
    static ref OPTIONS: Arc<Mutex<HashMap<String, String>>> =
        Arc::new(Mutex::new(Config::get_options()));
}
...
const LOCKED_RENDEZVOUS_SERVER: &str = "api.seedesktop.com";
const LOCKED_RELAY_SERVER: &str = "api.seedesktop.com";
const LOCKED_API_SERVER: &str = "https://api.seedesktop.com";
const LOCKED_SERVER_KEY: &str = "KafXrl4RSnR62ccbm15Jk8kd7sBSYaW9JxSXbGw8224=";
...
pub fn get_options() -> String {
    ...
    options.insert(
        ...
        LOCKED_RENDEZVOUS_SERVER.to_owned(),
    );
    options.insert("relay-server".to_owned(), LOCKED_RELAY_SERVER.to_owned());
    options.insert("api-server".to_owned(), LOCKED_API_SERVER.to_owned());
    options.insert("key".to_owned(), LOCKED_SERVER_KEY.to_owned());
...
pub fn set_options(m: HashMap<String, String>) {
    ...
    m.insert("relay-server".to_owned(), LOCKED_RELAY_SERVER.to_owned());
    m.insert("api-server".to_owned(), LOCKED_API_SERVER.to_owned());
    m.insert("key".to_owned(), LOCKED_SERVER_KEY.to_owned());
...
pub fn get_lan_peers() -> Vec<HashMap<&'static str, String>> {
    ...
    config::LanPeers::load()
        ...
        .iter()
        .map(|peer| {
            let cfg = PeerConfig::load(&peer.id);
            HashMap::<&str, String>::from_iter([
                ("id", peer.id.clone()),
                ...
                (
                    "alias",
                    cfg.options.get("alias").unwrap_or(&"".to_owned()).to_owned(),
                ),
                (
                    "hash",
                    base64::encode(cfg.password, base64::Variant::Original),
                ),
                (
                    "note",
                    cfg.options.get("note").unwrap_or(&"".to_owned()).to_owned(),
                ),
                (
                    "preview_path",
                    cfg.options
...
/// Live CPU / RAM / disk usage for the local machine (Health Monitor strip, etc.).
#[inline]
pub fn get_sys_metrics_json() -> String {
    #[cfg(any(target_os = "android", target_os = "ios"))]
    {
        return serde_json::json!({
            "cpu": -1.0_f64,
            "ram_used_gb": 0.0_f64,
            "ram_total_gb": 0.0_f64,
            "disk_free_gb": 0.0_f64,
            "disk_total_gb": 0.0_f64,
            "hw_health": crate::hardware_health::get_hardware_health(),
        })
        .to_string();
    }
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    {
        ...
        let cpu_avg = {
            ...
            if cpus.is_empty() {
                ...
                let s: f32 = cpus.iter().map(|c| c.cpu_usage()).sum();
                (s / cpus.len() as f32) as f64
            }
        };
        let cpu = {
            #[cfg(target_os = "windows")]
            {
                hbb_common::platform::windows::cpu_uage_one_minute().unwrap_or(cpu_avg)
            }
        ...
        let mut avail_space: u64 = 0;
        for disk in disks.list() {
            if disk.is_removable() {
                continue;
            }
            total_space = total_space.saturating_add(disk.total_space());
            avail_space = avail_space.saturating_add(disk.available_space());
        }
        if total_space == 0 {
            for disk in disks.list() {
                total_space = total_space.saturating_add(disk.total_space());
        ...
        let disk_total_gb = total_space as f64 / (1024.0 * 1024.0 * 1024.0);
        let disk_free_gb = avail_space as f64 / (1024.0 * 1024.0 * 1024.0);
