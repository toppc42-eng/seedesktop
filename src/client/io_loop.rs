                                ...
                                fs::JobType::Printer => {
                                    if let Some(err) = err {
                                        ...
                                        if let Some(data) = printer_data {
                                            ...
                                            std::thread::spawn(move || {
                                                log::info!(
                                                    "Dispatching printer bytes to OS print API: selected_printer='{}', data_len={}",
                                                    printer_name.clone().unwrap_or_default(),
                                                    data.len()
                                                );
                                                if let Err(e) =
                                            ...
                                            log::error!(
                                                "Receive print job completed but no payload buffer found (printer_data=None)"
                                            );
                                        }
                                    }
                                }
                    ...
                    Some(file_action::Union::Send(_s)) => match _s.file_type.enum_value() {
                        ...
                        Ok(file_transfer_send_request::FileType::Printer) => {
                            ...
                            if action == "dismiss" {
                                ...
                                log::info!(
                                    "Incoming printer request ignored by policy (dismiss): path={}",
                                    _s.path
                                );
                            } else {
                                ...
                                if allow_auto_print {
                                    ...
                                    log::info!(
                                        "Incoming printer request auto-accepted: id={}, path={}, printer='{}'",
                                        id,
                                        _s.path,
                                        printer_name
                                    );
                                    ...
                                    log::info!(
                                        "Incoming printer request requires UI confirmation: id={}, path={}",
                                        id,
                                        _s.path
                                    );
