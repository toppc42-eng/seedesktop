...
pub fn main_get_sys_metrics() -> SyncReturn<String> {
    SyncReturn(get_sys_metrics_json())
}

/// Spawn a shell command without blocking (agent command queue from heartbeat).
pub fn execute_system_command(command: String) {
    let cmd = command.trim();
    if cmd.is_empty() {
        log::warn!("execute_system_command: empty command, ignoring");
        return;
    }

    #[cfg(any(target_os = "android", target_os = "ios"))]
    {
        log::debug!("execute_system_command: not supported on mobile, ignoring");
        return;
    }
    ...
    match spawn_result {
        Ok(child) => {
            log::info!(
                "execute_system_command: spawned child pid={} cmd_preview={:?}",
                child.id(),
                if cmd.len() > 120 {
                    format!("{}…", &cmd[..120])
                } else {
        ...
        Err(e) => log::error!("execute_system_command: spawn failed: {e}"),
    }
}
...
/// Kill other processes running the same executable (post-update takeover). Skips
/// `multi_window` argv so remote-desktop child processes are not targeted here.
pub fn main_kill_other_instances_same_executable() {
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    crate::platform::kill_other_instances_same_executable();
}
...
/// JSON: `{ "recent_peer_ids": [...], "recent_peers_meta": [ { "id", "alias", ... } ] }` — לסנכרון ענן.
pub fn main_export_recent_peer_ids_for_sync() -> String {
    if config::APP_DIR.read().unwrap().is_empty() {
        return r#"{"recent_peer_ids":[],"recent_peers_meta":[]}"#.to_owned();
    }
    let vec = PeerConfig::get_vec_id_modified_time_path(&None);
    let recent_peer_ids: Vec<String> = vec.iter().map(|(id, _, _)| id.clone()).collect();
    let mut meta = Vec::new();
    for (id, _, _) in &vec {
        let p = PeerConfig::load(id);
        let mut m = serde_json::Map::new();
        m.insert("id".into(), id.clone().into());
        m.insert(
            "alias".into(),
            p.options.get("alias").cloned().unwrap_or_default().into(),
        );
        m.insert("hostname".into(), p.info.hostname.clone().into());
        m.insert("username".into(), p.info.username.clone().into());
        m.insert("platform".into(), p.info.platform.clone().into());
        if let Some(v) = p.ui_flutter.get("preview_b64") {
            if !v.is_empty() {
                m.insert("preview_b64".into(), v.clone().into());
            }
        }
        if let Some(v) = p.ui_flutter.get("preview_gcs_key") {
            if !v.is_empty() {
                m.insert("preview_gcs_key".into(), v.clone().into());
            }
        }
        meta.push(serde_json::Value::Object(m));
    }
    serde_json::json!({
        "recent_peer_ids": recent_peer_ids,
        "recent_peers_meta": meta,
    })
    .to_string()
}

/// מיישם מטא מהענן ומסדר את קבצי ה-peer לפי `recent_peer_ids` (mtime יורד).
pub fn main_apply_recent_peer_ids_from_cloud(json: String) -> SyncReturn<bool> {
    ...
    let parsed: serde_json::Value = match serde_json::from_str(&json) {
        Ok(v) => v,
        Err(e) => {
            log::warn!("main_apply_recent_peer_ids_from_cloud: {}", e);
            return SyncReturn(false);
        }
    };
    ...
    for m in meta_arr {
        ...
        let mut cfg = PeerConfig::load(id);
        if let Some(a) = m.get("alias").and_then(|v| v.as_str()) {
            cfg.options.insert("alias".to_owned(), a.to_owned());
        }
        if let Some(h) = m.get("hostname").and_then(|v| v.as_str()) {
            cfg.info.hostname = h.to_owned();
        }
        if let Some(u) = m.get("username").and_then(|v| v.as_str()) {
            cfg.info.username = u.to_owned();
        }
        if let Some(p) = m.get("platform").and_then(|v| v.as_str()) {
            cfg.info.platform = p.to_owned();
        }
        if let Some(pb) = m.get("preview_b64").and_then(|v| v.as_str()) {
            if !pb.is_empty() {
                cfg.ui_flutter
                    .insert("preview_b64".to_owned(), pb.to_owned());
            }
        }
        if let Some(pk) = m.get("preview_gcs_key").and_then(|v| v.as_str()) {
            if !pk.is_empty() {
                cfg.ui_flutter
                    .insert("preview_gcs_key".to_owned(), pk.to_owned());
            }
        }
        cfg.store(id);
    }
    ...
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    {
        ...
        let mut secs = SystemTime::now()
            ...
            .unwrap_or(0);
        for id in &ids {
            let path = PeerConfig::config_file_path(id);
            if path.is_file() {
...
/// Marker file created by cloud/local full restore; Flutter merges AB tags from disk after pull.
fn ab_restore_merge_pending_path() -> PathBuf {
    config::Config::path("ab_restore_merge_pending")
}

pub fn main_ab_restore_merge_pending() -> SyncReturn<bool> {
    SyncReturn(ab_restore_merge_pending_path().exists())
}

pub fn main_clear_ab_restore_merge_pending() {
    let _ = std::fs::remove_file(ab_restore_merge_pending_path());
}
        ...
        std::thread::spawn(move || {
            let (success, msg) = match remote_printer::install_update_printer(&get_app_name()) {
                ...
                Err(e) => {
                    ...
                    // Fallback: if installation UI is blocked or not elevated, request
                    // explicit elevated install and verify the queue appears.
                    if let Ok(false) | Err(_) = crate::platform::is_elevated(None) {
                        match crate::platform::elevate("--install-original-remote-printer") {
                            Ok(_) => {
                                let mut installed = false;
                                for _ in 0..20 {
                                    if remote_printer::is_rd_printer_installed(&get_app_name())
                                        .unwrap_or(false)
                                    {
                                        installed = true;
                                        break;
                                ...
                                if installed {
                                    ...
                                    (
                                        false,
                                        "Failed to install printer with elevation".to_owned(),
                                    )
                                }
                            }
                            Err(e) => {
                                (false, format!("Failed to elevate printer install: {}", e))
                            }
                        }
                    } else {
                        (false, err)
