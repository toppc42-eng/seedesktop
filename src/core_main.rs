...
pub fn core_main() -> Option<Vec<String>> {
    ...
    #[cfg(windows)]
    let is_quick_support_launch = args.is_empty() && is_quick_support_exe(&arg_exe);
    #[cfg(not(windows))]
    let is_quick_support_launch = false;
    #[cfg(windows)]
    let should_auto_install = args.is_empty()
        && !config::is_disable_installation()
        && !is_quick_support_launch
        && (click_setup || !crate::platform::is_installed());
    #[cfg(not(windows))]
    let should_auto_install = false;
    ...
    if args.is_empty() || crate::common::is_empty_uni_link(&args[0]) {
        ...
        #[cfg(windows)]
        {
            ...
            if args[0] == "--uninstall" {
                ...
                || args[0] == "--install-original-remote-printer"
            {
                #[cfg(windows)]
