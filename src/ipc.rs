...
/// When true, main-process IPC uses postfix `_quick_support` (SOS / portable, separate from installed service).
static MAIN_IPC_POSTFIX_QUICK_SUPPORT: AtomicBool = AtomicBool::new(false);

pub fn set_main_ipc_quick_support(active: bool) {
    MAIN_IPC_POSTFIX_QUICK_SUPPORT.store(active, Ordering::Relaxed);
}

pub fn main_ipc_quick_support_active() -> bool {
    MAIN_IPC_POSTFIX_QUICK_SUPPORT.load(Ordering::Relaxed)
}

pub fn main_ipc_postfix() -> &'static str {
    if main_ipc_quick_support_active() {
        "_quick_support"
    } else {
        ""
    }
...
async fn handle(data: Data, stream: &mut Connection) {
    match data {
        ...
        Data::PrinterData(data) => {
            log::info!(
                "IPC: received printer data from service process, len={}, dispatching to connections",
                data.len()
            );
