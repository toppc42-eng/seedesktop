...
pub const SCRAP_X11_REF_URL: &str = "https://seedesktop.com/docs/en/manual/linux/#x11-required";
...
impl LoginConfigHandler {
    ...
    pub fn restart_remote_device_safe_mode(&self) -> Message {
        let mut misc = Misc::new();
...
async fn do_sync_cpu_usage() {
    ...
    let start = std::time::Instant::now();
    match connect(50, crate::ipc::main_ipc_postfix()).await {
        Ok(mut conn) => {
...
lazy_static::lazy_static! {
    static ref LOGIN_ERROR_MAP: Arc<HashMap<&'static str, LoginErrorMsgBox>> = {
        ...
        let map = HashMap::from([(LOGIN_SCREEN_WAYLAND, LoginErrorMsgBox{
            ...
            link: "https://seedesktop.com/docs/en/manual/linux/#login-screen",
            try_again: true,
...
pub async fn handle_hash(
    ...
    // IS_TERMINAL_ADMIN no longer triggers a separate Windows-admin login dialog;
    // terminal uses the same SeeDesktop password flow as remote desktop.
